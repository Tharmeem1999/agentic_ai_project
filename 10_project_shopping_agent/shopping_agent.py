...
def search_products(
    ...
    Optionally filter by maximum price and/or organic status ("true" or "false").
    Returns a JSON array of matching products, each with: id, name, category,
    price, description, is_organic.
    """
    # 1. Parse is_organic input safely
    organic_bool: Optional[bool] = None
    if is_organic is not None:
        if isinstance(is_organic, str):
            organic_bool = is_organic.strip().lower() in ("true", "1", "yes")
        elif isinstance(is_organic, bool):
            organic_bool = is_organic

    # 2. Parse max_price input safely
    ...
    if organic_bool is not None:
        sql += " AND is_organic = ?"
        params.append(1 if organic_bool else 0)  # Matches 0s and 1s in DB
    ...
    products = [
        {
            ...
            "price": row[3],
            "description": row[4],
            "is_organic": bool(row[5]),  # Converts 1 -> True, 0 -> False
        }
        for row in rows
    ]
...
def get_rating(product_id: int) -> str:
    ...
    return json.dumps(result)  # Fixed: json.dumps
...
def checkout(product_id: int) -> str:
    ...
    Saves the order to the database and returns a confirmation message with the
    order ID, product name, and price.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    # Fixed: (product_id,) tuple
    cursor.execute(
